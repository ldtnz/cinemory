...
/**
 * The animated terminal shown behind the sign-in card when there is no
 ...
 *    default, which a server render evaluates too, and there is no window
 *    there. It is read inside the effect instead.
 *  - the whole thing is skipped when the reader asked for reduced motion:
 *    this is decoration on a page whose only job is a six-digit field, and
 *    src/app/globals.css already honours that preference everywhere else.
 *  - the random time offset moved out of render, where React's rules forbid
 *    an impure call.
...
export default function FaultyTerminal({
  ...
  // Seeded in the effect rather than here: a random number during render is
  // an impure call, which React's lint rules refuse outright — and the offset
  // is only read once the animation starts.
  ...
  useEffect(() => {
    ...
    try {
      renderer = new Renderer({ dpr: dpr ?? Math.min(window.devicePixelRatio || 1, 2) });
    } catch {
      // No WebGL: the card is legible on the plain background behind it.
      return;
    }
